...
### 1. Simple Secret Key Setup (AES-256-GCM)
Configure your secret key globally or locally (must be 32 bytes or 64 character hex string). The interceptor will automatically encrypt the `data` payload and format it as `iv:auth_tag:ciphertext`:

```javascript
app.use(responseInterceptor({
  encrypt: { secretKey: 'your-32-character-secret-key-here!!!' }
}));
```

### 2. Custom Encrypt Function Callback
You can supply a custom encryption callback (e.g. to run custom public-key / hybrid encryption):

```javascript
app.use(responseInterceptor({
  encrypt: (data) => {
    // Custom encryption logic
    return customEncrypt(JSON.stringify(data));
  }
}));
```
...
You can plug in your Winston or Pino logging instance to capture details and durations of requests, responses, and errors:
